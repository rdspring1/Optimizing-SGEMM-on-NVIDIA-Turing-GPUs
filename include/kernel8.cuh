...
__global__ __launch_bounds__(256) void mysgemm_v8(int M, int N, int K,
  ...
  for (int k_count = 0; k_count < K; k_count += KS_8) {
    vload(Av1, &A(row_a, col_a))
        vload(Bv1, &B(row_b, col_b))((float4 *)sa8)[tx] = Av1;
...
#pragma unroll
    for (int inner_k_count = 0; inner_k_count < KS_8; inner_k_count++) {
      vload(Av1, &sa8(row_c, inner_k_count)) vload(
          Av2, &sa8(row_c + 4, inner_k_count)) vload(Bv1,
                                                     &sb8(col_c, inner_k_count))
          vload(Bv2, &sb8(col_c + 4, inner_k_count)) vscal(Cres[0], Av1, Bv1.x)
              vscal(Cres[1], Av2, Bv1.x) vscal(Cres[2], Av1, Bv1.y)
                  vscal(Cres[3], Av2, Bv1.y) vscal(Cres[4], Av1, Bv1.z) vscal(
                      Cres[5], Av2, Bv1.z) vscal(Cres[6], Av1, Bv1.w)
                      vscal(Cres[7], Av2, Bv1.w) vscal(Cres[8], Av1, Bv2.x)
                          vscal(Cres[9], Av2, Bv2.x) vscal(Cres[10], Av1, Bv2.y)

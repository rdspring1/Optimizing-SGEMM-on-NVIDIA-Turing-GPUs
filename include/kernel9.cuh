...
__global__ __launch_bounds__(256) void mysgemm_v9(int M, int N, int K,
  ...
  for (int k_count = 0; k_count < K; k_count += KS_9) {
    ...
    vload(Av1, &A(row_a, col_a))
        vload(Bv1, &B(row_b, col_b))((float4 *)sa9)[tx] = Av1;
...
#pragma unroll
    for (int inner_k_count = 0; inner_k_count < KS_9; inner_k_count++) {
      vload(Av1, &sa9(row_c, inner_k_count)) vload(
          Av2, &sa9(row_c + 4, inner_k_count)) vload(Bv1,
                                                     &sb9(col_c, inner_k_count))
          vload(Bv2, &sb9(col_c + 4, inner_k_count)) vscal(Cres[0], Av1, Bv1.x)
